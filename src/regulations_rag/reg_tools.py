...
def check_columns(manual_as_df):
    ''' 
    ...
    actual_columns = manual_as_df.columns.to_list()
...
def get_regulation_heading(node_str, df, section_index_tracker):
    if not section_index_tracker.is_valid(node_str):
        return "Not a valid reference"
    remaining_reference = node_str
    heading = ""
    
    while len(remaining_reference) > 0:
        tmp_df = df.loc[(df['section_reference'] == remaining_reference) & (df['heading'] == True)]
        if len(tmp_df) > 1:
            return f"There was more than one heading for the section reference {node_str}"
        elif len(tmp_df) == 1:
            if heading == "":
                heading = tmp_df.iloc[0]["reference"] + " " + tmp_df.iloc[0]["text"] + "."
            else:
                heading = tmp_df.iloc[0]["reference"] + " " + tmp_df.iloc[0]["text"] + ". " + heading
        remaining_reference = section_index_tracker.get_parent_reference(remaining_reference)
...
#       a dataframe form a file without the 'na_filter=False' option. You should ensure that the dataframe does 
#       not have any NaN value for the text fields. Try running df.isna().any().any() as a test before you get here
def get_regulation_detail(node_str, df, section_index_tracker):
    # if not section_index_tracker.is_valid(node_str):
    #     return "The reference did not conform to this documents standard"
    text = ''
    terminal_text_df = df[df['section_reference'].str.startswith(node_str)]
    ...
    if node_str != '': #i.e. there is a parent
        parent_reference = section_index_tracker.get_parent_reference(node_str)
        all_conditions = ""
        all_qualifiers = ""
        while parent_reference != "":
            parent_text_df = df[df['section_reference'] == parent_reference]
            conditions = ""
            qualifiers = ""
            ...
            parent_reference = section_index_tracker.get_parent_reference(parent_reference)
