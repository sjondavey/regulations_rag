...
from regulations_rag.path_rag import format_user_question, \
                                                 create_system_message_RAG, \
                                                 check_response_RAG, \
                                                 RAGPath, \
                                                 LLMPrefix, \
                                                 add_section_to_resource, \
...
def test_format_user_question(dummy_definitions, dummy_search_sections):
    ...
    output_string = format_user_question(question, dummy_definitions, dummy_search_sections)
...
def test_create_system_message():
    ...
    assert create_system_message_RAG(corpus_index, number_of_options=3, review = False) == expected_message
    ...
    assert create_system_message_RAG(corpus_index, number_of_options=2, review = False) == expected_message
    ...
    assert create_system_message_RAG(corpus_index, number_of_options=3, review = True) == expected_message
    ...
    assert create_system_message_RAG(corpus_index, number_of_options=2, review = True) == expected_message

def test_check_response_RAG(dummy_definitions, dummy_search_sections):
    # Setup
    corpus_index = NavigatingIndex()
    corpus = corpus_index.corpus

    reference_key_word = "Reference:"
    # Test Case where the reference_key_word appears more than once
    
    llm_response_text = f"{LLMPrefix.ANSWER.value} This is part of an answer. Reference: 1, 2. \n\nThen here is more of the answer. Reference: 3, 4"
    llm_message_response = {"role": "assistant", "content": llm_response_text}
    result = check_response_RAG(corpus = corpus, llm_message_response  = llm_message_response, df_definitions = dummy_definitions, df_sections = dummy_search_sections)
    assert result["RAGPath"] == RAGPath.FOLLOWUP.value
    ...
    llm_response = f"{LLMPrefix.ANSWER.value} This is a valid answer without references."
    ...
    llm_response = f"{LLMPrefix.ANSWER.value} This is a valid answer. {reference_key_word} 1, 2"
    llm_message_response = {"role": "assistant", "content": llm_response}
    result = check_response_RAG(corpus = corpus, llm_message_response = llm_message_response, df_definitions = dummy_definitions, df_sections = dummy_search_sections)
    assert "assistant_response" in result
    ...
    llm_response = f"{LLMPrefix.ANSWER.value} This is a valid answer. {reference_key_word} A"
    llm_message_response = {"role": "assistant", "content": llm_response}
    result = check_response_RAG(corpus = corpus, llm_message_response = llm_message_response, df_definitions = dummy_definitions, df_sections = dummy_search_sections)
    assert result["RAGPath"] == RAGPath.FOLLOWUP.value
    ...
    llm_response = f"{LLMPrefix.ANSWER.value} This is a valid answer. {reference_key_word} 10.0"
    llm_message_response = {"role": "assistant", "content": llm_response}
    result = check_response_RAG(corpus = corpus, llm_message_response = llm_message_response, df_definitions = dummy_definitions, df_sections = dummy_search_sections)
    assert result["RAGPath"] == RAGPath.FOLLOWUP.value
    ...
    llm_response = f"{LLMPrefix.SECTION.value} Extract 1, Reference 1.1"
    llm_message_response = {"role": "assistant", "content": llm_response}
    result = check_response_RAG(corpus = corpus, llm_message_response = llm_message_response, df_definitions = dummy_definitions, df_sections = dummy_search_sections)
    assert result["RAGPath"] == RAGPath.SECTION.value
    ...
    llm_response = f"{LLMPrefix.SECTION.value} Extract 1, Reference: 1.1.2"
    llm_message_response = {"role": "assistant", "content": llm_response}   
    result = check_response_RAG(corpus = corpus, llm_message_response = llm_message_response, df_definitions = dummy_definitions, df_sections = dummy_search_sections)
    assert result["RAGPath"] == RAGPath.FOLLOWUP.value
    ...
    llm_response = f"{LLMPrefix.NONE.value} This is a valid answer. {reference_key_word} 1, 2"
    llm_message_response = {"role": "assistant", "content": llm_response}
    result = check_response_RAG(corpus = corpus, llm_message_response = llm_message_response, df_definitions = dummy_definitions, df_sections = dummy_search_sections)
    assert "assistant_response" in result
...
def test_add_section_to_resource(dummy_definitions, dummy_search_sections):
    ...
    result = {"success": True, "RAGPath": RAGPath.SECTION.value, "extract": 1, "document": 'WRR', "section": "9.1"}
    success, df_updated = add_section_to_resource(corpus = corpus, result = result, df_definitions = dummy_definitions, df_search_sections = dummy_search_sections)
    ...
    result = {"success": True, "RAGPath": RAGPath.SECTION.value, "extract": 5, "document": 'WRR', "section": "1.1"}
    success, df_updated = add_section_to_resource(corpus = corpus, result = result, df_definitions = dummy_definitions, df_search_sections = dummy_search_sections)
    assert success == True
    ...
    result = {"success": True, "RAGPath": RAGPath.SECTION.value, "extract": 1, "document": 'WRR', "section": "1.1"}
    success, df_updated = add_section_to_resource(corpus = corpus, result = result, df_definitions = dummy_definitions, df_search_sections = dummy_search_sections)
    assert len(df_updated) == 4
    assert success == True
    assert df_updated.iloc[0]['section_reference'] == "1.2"
...
def test_process_llm_response(dummy_definitions, dummy_search_sections):
    ...
    chat_data = ChatDataForRAG(
        corpus_index=corpus_index,  # We don't need this for the test
    ...
    check_response = check_response_RAG(corpus = chat_data.corpus, llm_message_response = llm_message_response, df_definitions = dummy_definitions, df_sections = dummy_search_sections)
    assert check_response["RAGPath"] == RAGPath.FOLLOWUP.value
    assert check_response["llm_followup_instruction"] == f"Your response, did not begin with one of the keywords, '{LLMPrefix.ANSWER.value}', '{LLMPrefix.SECTION.value}' or '{LLMPrefix.NONE.value}'. Please review the question and provide an answer in the required format. Also make sure the referenced extracts are quoted at the end of the answer, not in the body, by number, in a comma separated list starting after the keyword '{reference_key_word_RAG}'. Do not include the word Extract, only provide the number(s).\n"
    # Create a mock for the get_api_response method
    mock_get_api_response = MagicMock(return_value=f"{LLMPrefix.ANSWER.value} But the followup call to the API then fixes it: This is a test answer. Reference: 1, 2")
    # Patch the get_api_response method of the ChatParameters instance
    with patch.object(chat_data.chat_parameters, 'get_api_response', mock_get_api_response):
        result = process_llm_response(chat_data, check_response)
    assert "assistant_response" in result
    assert isinstance(result["assistant_response"], AnswerWithRAGResponse)
    ...
    section_response = f"{LLMPrefix.SECTION.value} Extract 1, Reference 1.1"
    llm_message_response = {"role": "assistant", "content": section_response}
    checked_response = check_response_RAG(corpus = chat_data.corpus, llm_message_response = llm_message_response, df_definitions = dummy_definitions, df_sections = dummy_search_sections)
    mock_resource_augmented_query = MagicMock(return_value={"role": "assistant", "content": f"{LLMPrefix.ANSWER.value} OK, I was able to answer with the new reference. Reference: 1, 2"})    
    # Use patch as a context manager
    with patch('regulations_rag.path_rag.resource_augmented_query', mock_resource_augmented_query):
        result = process_llm_response(chat_data, checked_response)
    assert isinstance(result["assistant_response"], AnswerWithRAGResponse)
    ...
    llm_message_response = {"role": "assistant", "content": section_response}
    checked_response = check_response_RAG(corpus = chat_data.corpus, llm_message_response = llm_message_response, df_definitions = dummy_definitions, df_sections = dummy_search_sections)
    mock_resource_augmented_query = MagicMock(return_value={"role": "assistant", "content": f"{LLMPrefix.SECTION.value} Extract 1, Reference 2.1"})
    # Use patch as a context manager
    with patch('regulations_rag.path_rag.resource_augmented_query', mock_resource_augmented_query):        
        result = process_llm_response(chat_data, checked_response)
...
def test_extract_used_references(dummy_definitions, dummy_search_sections):
    ...
    df_result = extract_used_references(corpus = corpus, integer_list_of_used_references = result["reference"], df_definitions = dummy_definitions, df_search_sections = dummy_search_sections)
    ...
    with pytest.raises(IndexError):
        extract_used_references(corpus = corpus, integer_list_of_used_references = result["reference"], df_definitions = dummy_definitions, df_search_sections = dummy_search_sections)
...
def test_perform_RAG_path(dummy_definitions, dummy_search_sections):
    ...
    chat_data_for_search = ChatDataForSearch(corpus_index = corpus_index, 
                                             chat_parameters = chat_parameters, 
    ...
    workflow_triggered, relevant_definitions, relevant_sections = similarity_search(chat_data_for_search, user_content)
    user_message = {"content": user_content, "reference_material": {"definitions": relevant_definitions, "sections": relevant_sections}}

    chat_data_for_rag = ChatDataForRAG(corpus_index = corpus_index, 
                                       chat_parameters = chat_parameters, 
                                       messages = [], 
                                       user_message = user_message)
    ...
    # Use patch as a context manager
    with patch.object(chat_data_for_rag.chat_parameters, 'get_api_response', mock_get_api_response):
        result = perform_RAG_path(chat_data_for_rag)
