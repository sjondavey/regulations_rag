...
class TestSectionReferenceChecker:
    ...
    def test_is_valid(self):
        blank_reference = ""
        assert not self.section_reference_checker.is_valid(blank_reference)

        long_reference = 'G.1(C)(xviii)(c)(dd)(9)'
        assert self.section_reference_checker.is_valid(long_reference)
        very_long_reference = 'G.1(C)(xviii)(c)(dd)(9)(10)'
        assert not self.section_reference_checker.is_valid(very_long_reference)

        short_reference = 'G.1(C)'        
        assert self.section_reference_checker.is_valid(short_reference)

        reference_on_exclusion_list = 'Legal context'
        assert self.section_reference_checker.is_valid(reference_on_exclusion_list)

        invalid_reference = 'G.1(C)(xviii)(c)(c)(9)'
        assert not self.section_reference_checker.is_valid(invalid_reference)
    ...
    def test_extract_valid_reference(self):
        assert self.section_reference_checker.extract_valid_reference('B.18 Gold (B)(i)(b)') == 'B.18(B)(i)(b)'
        assert self.section_reference_checker.extract_valid_reference('   B.18 Gold (B)(i)(b)') == 'B.18(B)(i)(b)'
        #assert self.section_reference_checker.extract_valid_reference('B.18 Gold (B)(i)(ii)')  is None
        assert self.section_reference_checker.extract_valid_reference('B.18 Gold (B)(i)(ii)')  == 'B.18(B)(i)'
        assert self.section_reference_checker.extract_valid_reference('A.1') == 'A.1'
        assert self.section_reference_checker.extract_valid_reference('B.18 Gold (B)(i)(b) hello') == 'B.18(B)(i)(b)' # text at the end
        #assert self.section_reference_checker.extract_valid_reference('B.18 Gold (B)(i)(b) (hello)') == None  # the text at the end contains an "("
        assert self.section_reference_checker.extract_valid_reference('B.18 Gold (B)(i)(b) (hello)') == 'B.18(B)(i)(b)'

    def test_split_reference(self):
        long_reference = 'G.1(C)(xviii)(c)(dd)(9)'
        components = self.section_reference_checker.split_reference(long_reference)
        ...
        short_reference = 'G.1(C)'        
        components = self.section_reference_checker.split_reference(short_reference)
        ...
        with pytest.raises(ValueError):
            components = self.section_reference_checker.split_reference(invalid_reference)
        ...
        reference_on_exclusion_list = 'Legal context'
        components = self.section_reference_checker.split_reference(reference_on_exclusion_list)
        assert components[0] == reference_on_exclusion_list

    def test_get_parent_reference(self):
        reference = 'G.1(C)(xviii)(c)(dd)(9)'
        assert self.section_reference_checker.get_parent_reference(reference) == 'G.1(C)(xviii)(c)(dd)'
        with pytest.raises(ValueError):
            components = self.section_reference_checker.get_parent_reference("")
    ...
    def test_get_current_and_parent_references(self):
        ...
        assert self.section_reference_checker.get_current_and_parent_references(reference) == current_and_parent

    def test_is_reference_or_parents_in_list(self):
        reference = 'G.1(C)(xviii)(c)(dd)(9)'
        list_of_references = ['A.1', 'B.1', 'C.1']
        assert not self.section_reference_checker.is_reference_or_parents_in_list(reference, list_of_references)
        list_of_references = ['A.1', 'B.1', 'G.1']
        assert self.section_reference_checker.is_reference_or_parents_in_list(reference, list_of_references)
    ...
    def test_parse_line_of_text(self):
        ...
        with pytest.raises(ValueError):
            indent, index, remaining_text = self.section_reference_checker.parse_line_of_text(string_with_incorrect_indent)

        string_with_mismatched_indent_and_index = "                (c) the name and registration number of the applicant company; "
        with pytest.raises(ValueError):
            indent, index, remaining_text = self.section_reference_checker.parse_line_of_text(string_with_mismatched_indent_and_index)

        string_with_correct_indent = "                (aa) the name and registration number of the applicant company; "
        indent, index, remaining_text = self.section_reference_checker.parse_line_of_text(string_with_correct_indent)
        assert indent == 4
        assert index == '(aa)'
        assert remaining_text == 'the name and registration number of the applicant company; '
        
        reference_on_exclusion_list = 'Legal context'
        indent, index, remaining_text = self.section_reference_checker.parse_line_of_text(reference_on_exclusion_list)
        assert indent == 0
        ...
        with pytest.raises(ValueError):
            indent, index, remaining_text = self.section_reference_checker.parse_line_of_text(reference_on_exclusion_list_wrong_indent)
    ...
    def test___extract_reference_from_string(self):
        ...
        index, string = self.section_reference_checker._extract_reference_from_string(string_with_no_reference)
        assert index == ""
        ...
        index, string = self.section_reference_checker._extract_reference_from_string(string_with_reference)
        assert index == "A.1"
        ...
        index, string = self.section_reference_checker._extract_reference_from_string(heading_on_exclusion_list)
