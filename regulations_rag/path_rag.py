...
#
# 1) resource_augmented_query(...): This takes ChatDataForRAG and number_of_options as input and returns a a message:
...
# a class to hold the data for the RAG path
class ChatDataForRAG:
    def __init__(self, corpus_index, chat_parameters, messages, user_message):
        ...
        self.df_definitions = user_message["reference_material"]["definitions"]
        self.df_search_sections = user_message["reference_material"]["sections"]


def perform_RAG_path(chat_data_for_rag):
    """
    ...
    Parameters:
    - chat_data_for_rag (ChatDataForRAG): An object containing all necessary data for the RAG process,
      including corpus information, chat parameters, messages, and dataframes for definitions and search sections.
    ...
    """
    rag_response = resource_augmented_query(chat_data = chat_data_for_rag, 
                                            number_of_options = 3)
    if "assistant_response" in rag_response:
        return rag_response

    llm_checked_response = check_response_RAG(corpus = chat_data_for_rag.corpus, 
                                              llm_message_response = rag_response, 
                                              df_definitions = chat_data_for_rag.df_definitions, 
                                              df_sections = chat_data_for_rag.df_search_sections)
    if "assistant_response" in llm_checked_response:
        return llm_checked_response

    final_response = process_llm_response(chat_data = chat_data_for_rag, 
                                         llm_checked_response = llm_checked_response)
...
def format_user_question(question, df_definitions, df_search_sections):
    """
...
def create_system_message_RAG(corpus_index, number_of_options=3, review=False):
    """
    ...
    """
    if not review:
        sys_instruction = f"You are answering questions about {corpus_index.corpus_description} for {corpus_index.user_type} based only on the reference extracts provided. You have {number_of_options} options:\n"
    else:
    ...
    if corpus_index.corpus.get_primary_document() != "":
        primary_document = corpus_index.corpus.get_document(corpus_index.corpus.get_primary_document())
        sample_reference = primary_document.reference_checker.text_version
    else:
        sample_reference = "[Insert Reference Value Here]"

    sys_option_ans  = f"Answer the question. Preface an answer with the tag '{LLMPrefix.ANSWER.value}'. All referenced extracts must be quoted at the end of the answer, not in the body, by number, in a comma separated list starting after the keyword '{reference_key_word_RAG}'. Do not include the word Extract, only provide the number(s).\n"
    sys_option_sec  = f"Request additional documentation. If, in the body of the extract(s) provided, there is a reference to another section that is directly relevant and not already provided, respond with the word '{LLMPrefix.SECTION.value}' followed by 'Extract extract_number, {reference_key_word_RAG} section_reference' - for example SECTION: Extract 1, {reference_key_word_RAG} {sample_reference}.\n"
    sys_option_none = f"State '{LLMPrefix.NONE.value}' and nothing else in all other cases\n"
...
def check_response_RAG(corpus, llm_message_response, df_definitions, df_sections):
    """
    ...
    if full_llm_text_response.startswith(LLMPrefix.ANSWER.value):
        prefix = LLMPrefix.ANSWER.value
        ...
        count = llm_text_without_prefix.count(reference_key_word_RAG)
        if count > 1:
            llm_instruction = f"When answering the question, you used the keyword '{reference_key_word_RAG}' more than once. It is vitally important that this keyword is only used once in your answer and then only at the end of the answer followed only by an integer, comma separated list of the extracts used. Please reformat your response so that there is only one instance of the keyword '{reference_key_word_RAG}' and it is at the end of the answer."
            return {
                "RAGPath": RAGPath.FOLLOWUP.value, 
        ...
        references = llm_text_without_prefix.split(reference_key_word_RAG)[-1].split(",") if reference_key_word_RAG in llm_text_without_prefix else []
        cleaned_references = [ref.strip() for ref in references if ref.strip()]
        llm_text = llm_text_without_prefix[:llm_text_without_prefix.rfind(reference_key_word_RAG)].strip() if reference_key_word_RAG in llm_text_without_prefix else llm_text_without_prefix
        ...
        df_used_references = extract_used_references(corpus = corpus, 
    ...
    elif full_llm_text_response.startswith(LLMPrefix.SECTION.value):
        prefix = LLMPrefix.SECTION.value
        ...
        if match:
            ...
            doc = corpus.get_document(document_name)
            ...
            if doc.reference_checker.is_valid(section_reference):
            ...
            elif corpus.get_primary_document() != "" and document_name != corpus.get_primary_document(): # articles in other documents can refer to the primary document so it makes sense to check if it is a primary document reference as well
                primary_doc = corpus.get_document(corpus.get_primary_document())
                if primary_doc.reference_checker.is_valid(section_reference):
                    ...
                    return {
                        "RAGPath": RAGPath.SECTION.value, 
                        "extract": extract_number, 
                        "document": corpus.get_primary_document(), 
                        "section": section_reference
                    }
    ...
    elif full_llm_text_response.startswith(LLMPrefix.NONE.value):
        return {
            "role": "assistant", 
            "content": "The system was not able to answer the question using the provided documents", 
    ...
    llm_instruction = f"Your response, did not begin with one of the keywords, '{LLMPrefix.ANSWER.value}', '{LLMPrefix.SECTION.value}' or '{LLMPrefix.NONE.value}'. Please review the question and provide an answer in the required format. Also make sure the referenced extracts are quoted at the end of the answer, not in the body, by number, in a comma separated list starting after the keyword '{reference_key_word_RAG}'. Do not include the word Extract, only provide the number(s).\n"
    return {
        "RAGPath": RAGPath.FOLLOWUP.value, 
        "content": full_llm_text_response,
        "llm_followup_instruction": llm_instruction
...
def resource_augmented_query(chat_data, number_of_options = 3):
    '''
    ...
    Parameters:
    - chat_data (ChatDataForRAG): An object containing all necessary data for the RAG process.
    ...
    '''
    if  len(chat_data.df_definitions) + len(chat_data.df_search_sections) == 0:
    ...
    system_content = create_system_message_RAG(chat_data.corpus_index, number_of_options, review=False)
    ...
    user_question = format_user_question(chat_data.user_question, chat_data.df_definitions, chat_data.df_search_sections)
    logger.log(DEV_LEVEL, "resource_augmented_query called with User Prompt:\n" + user_question) 

    local_messages = chat_data.messages.copy()
    ...
    #truncated_chat = truncate_message_list(system_message, local_messages, chat_data.chat_parameters.token_limit_when_truncating_message_queue)
    text_llm_response = chat_data.chat_parameters.get_api_response(system_message, local_messages)
...
def process_llm_response(chat_data, llm_checked_response):
    """
    ...
    Parameters:
    - chat_data (ChatDataForRAG): An object containing all necessary data for the RAG process,
      including corpus information, chat parameters, messages, and dataframes for definitions
      and search sections.
    ...
    if check_result["RAGPath"] == RAGPath.SECTION.value:
        success, df_updated = add_section_to_resource(corpus = chat_data.corpus, result = check_result, df_definitions=chat_data.df_definitions, df_search_sections=chat_data.df_search_sections)
        if success:
            chat_data.df_search_sections = df_updated
            updated_response = resource_augmented_query(chat_data, number_of_options = 3)
            result = check_response_RAG(corpus=chat_data.corpus, llm_message_response=updated_response, df_definitions=chat_data.df_definitions, df_sections=chat_data.df_search_sections)
    ...
    if check_result["RAGPath"] == RAGPath.FOLLOWUP.value:
        updated_messages = chat_data.messages.copy()
        updated_messages.append({"role": "user", "content": chat_data.user_question})
        ...
        response = chat_data.chat_parameters.get_api_response(system_message, updated_messages)
        llm_response_message = {"role": "assistant", "content": response}
        result = check_response_RAG(corpus=chat_data.corpus, llm_message_response=llm_response_message, df_definitions=chat_data.df_definitions, df_sections=chat_data.df_search_sections)
        if "assistant_response" in result:
            return result
...
def add_section_to_resource(corpus, result, df_definitions, df_search_sections):
    '''
    ...
    if result["RAGPath"] != RAGPath.SECTION.value:
    ...
    doc = corpus.get_document(result["document"])
...
def extract_used_references(corpus, integer_list_of_used_references, df_definitions, df_search_sections):
    ''' 
    ...
    for reference in integer_references:
        if reference <= number_of_definitions:
            ...
            document_name = corpus.get_document(df_definitions.iloc[row_number]["document"]).name
            ...
            document_name = corpus.get_document(document_key).name
            section_reference = df_search_sections.iloc[row_number]["section_reference"]                
            text = corpus.get_text(document_key, section_reference, add_markdown_decorators=True, add_headings=True, section_only=False)
